...
team_t team = {
    ...
    "Eliana Kang",     /* First member full name */
    "e.kang@gwmail.gwu.edu",  /* First member email address */
    ""               /* GWID */
...
for (ii = 0; ii < dim; ii += TILE_SIZE)
  for (jj = 0; jj < dim; jj += TILE_SIZE)
    for (j = jj; j < min(dim, jj + TILE_SIZE); j++)
      for (i = ii; i < min(dim, ii + TILE_SIZE); i += 8) {
        dst[RIDX(dim-1-j, i, dim)] = src[RIDX(i, j, dim)];
...
for (i = 1; i < dim-1; i++) {
  for (j = 1; j < dim-1; j++) {
    pixel *new_pixel = &dst[RIDX(i, j, dim)];
    int red_sum = 0, green_sum = 0, blue_sum = 0;
    
        red_sum += src[RIDX(i-1, j-1, dim)].red;
        green_sum += src[RIDX(i-1, j-1, dim)].green;
	    blue_sum += src[RIDX(i-1, j-1, dim)].blue;
        
        red_sum += src[RIDX(i-1, j, dim)].red;
        green_sum += src[RIDX(i-1, j, dim)].green;
	    blue_sum += src[RIDX(i-1, j, dim)].blue;
        
        red_sum += src[RIDX(i-1, j+1, dim)].red;
        green_sum += src[RIDX(i-1, j+1, dim)].green;
	    blue_sum += src[RIDX(i-1, j+1, dim)].blue;
        
        red_sum += src[RIDX(i, j-1, dim)].red;
        green_sum += src[RIDX(i, j-1, dim)].green;
	    blue_sum += src[RIDX(i, j-1, dim)].blue;
        
        red_sum += src[RIDX(i, j, dim)].red;
        green_sum += src[RIDX(i, j, dim)].green;
	    blue_sum += src[RIDX(i, j, dim)].blue;
        ...
        new_pixel->red = red_sum/9;
        new_pixel->green = green_sum/9;  
        new_pixel->blue = blue_sum/9;   
  }
 }
...
for (i = 0; i < dim; i++) {
  for (j = 0; j < dim; j++) {
    if (i > 0 && i < dim-1 && j > 0 && j < dim-1) {
      ...
      for (ii = maximum(i-1, 0); ii <= minimum(i+1, dim-1); ii++) {
        for (jj = maximum(j-1, 0); jj <= minimum(j+1, dim-1); jj++) {
          red_sum += src[RIDX(ii,jj,dim)].red;
          green_sum += src[RIDX(ii,jj,dim)].green;
          blue_sum += src[RIDX(ii,jj,dim)].blue;
          num++;
        }
      }
      
      new_pixel->red = red_sum/num;
      new_pixel->green = green_sum/num;  
      new_pixel->blue = blue_sum/num;
  }
}
